...
struct StateTransform<:Transform
    ...
    StateTransform(input::Ket{B, T}, output::Ket{B,T}) where {B,T} = new(input.basis, input, output)
...
mutable struct UnitaryTransform<:Transform
    ...
    UnitaryTransform(input::Ket{B, T}, output::Ket{B,T}) where {B,T} = new(input.basis, [input], [output])
    ...
    function UnitaryTransform(inputs::Vector{Ket{B,T}}, U::Matrix) where{B,T}
        ...
        length(inputs) == size(U)[1] || throw("Matrix dimensions do not correspond to the input kets$(U*U')")
        outputs = Ket{B,T}[]
        for r in eachrow(U)
            output = 0.0*inputs[1]
...
function Base.:+(a::UnitaryTransform, p::Pair{Ket{B,T}, Ket{B,T}}) where {B,T}
    ...
    for k in a.inputs
        dagger(k)*p[1] == 0.0 || @warn("The transformation is not unitary $(dagger(k)*p[1])")
    end
    for k in a.outputs
        dagger(k)*p[2] == 0.0 || @warn("The transformation is not unitary $(dagger(k)*p[2])")
    end
    push!(a.inputs, p[1])
    push!(a.outputs, p[2])
    a
...
function CuUnitaryTransform(trans::UnitaryTransform)
...
function StateTransform(trans::StateTransform)
